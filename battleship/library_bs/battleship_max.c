#include "battleship_max.h"

// ========= Constantes internas =========
static const uint32_t BLINK_MS      = 180;  // blink tablero jugador
static const uint32_t OPP_BLINK_MS  = 300;  // blink MISSES oponente
static const uint32_t ERR_BLINK_MS  = 90;   // periodo blink error
static const uint8_t  ERR_BLINK_TOGGLES = 6; // 3 ciclos on/off

// ========= Estado global =========

// Tableros lógicos
static BATTLESHIP_STATUS_Type player_board[8][8];
static BATTLESHIP_STATUS_Type opponent_board[8][8];

// Buffers de display
static uint8_t piv_rows[8];    // dev0
static uint8_t player_rows[8]; // dev1

// Oponente
static bool opponent_exists   = true;
static bool oppBlinkState     = true;
static uint32_t oppLastBlink  = 0;

// Contador (dev3)
static const uint8_t digits[10][8] = {
    {0x3C,0x66,0x6E,0x76,0x66,0x66,0x66,0x3C}, //0
    {0x18,0x38,0x78,0x18,0x18,0x18,0x18,0x7E}, //1
    {0x3C,0x66,0x06,0x0C,0x18,0x60,0x66,0x7E}, //2
    {0x3C,0x66,0x06,0x1C,0x06,0x06,0x66,0x3C}, //3
    {0x0C,0x1C,0x3C,0x6C,0x7E,0x0C,0x0C,0x1E}, //4
    {0x7E,0x60,0x7C,0x06,0x06,0x06,0x66,0x3C}, //5
    {0x3C,0x66,0x60,0x7C,0x66,0x66,0x66,0x3C}, //6
    {0x7E,0x66,0x06,0x0C,0x18,0x18,0x18,0x18}, //7
    {0x3C,0x66,0x66,0x3C,0x66,0x66,0x66,0x3C}, //8
    {0x3C,0x66,0x66,0x3E,0x06,0x06,0x66,0x3C}  //9
};
static uint8_t  current_digit = 9;

// Colocación
static int   prow = 3, pcol = 0;               // cursor barco / disparo
static ORI_t cur_ori = ORI_H;
static const uint8_t SHIP_LIST[3] = {2,4,6};  // barcos 2,4,6
static uint8_t ship_index = 0;                // 0->2, 1->4, 2->6
static bool all_ships_placed = false;
static BS_Mode mode = MODE_PLACE;

// Blink tablero jugador cuando terminó de colocar
static bool blink_enabled = false;
static bool blink_state   = false;
static uint32_t last_blink = 0;

// Blink de error al intentar colocar barco inválido
static bool     errBlink_active   = false;
static uint8_t  errBlink_togglesRemaining = 0;
static bool     errBlink_showBase = false; // alterna jugador vs preview
static uint32_t errBlink_last     = 0;

// ========= Helpers de tablero =========

static void boardClear(BATTLESHIP_STATUS_Type b[8][8]){
    for(int r=0;r<8;r++)
        for(int c=0;c<8;c++)
            b[r][c]=WATER;
}

static void boardToRowsPlayer(const BATTLESHIP_STATUS_Type b[8][8],
                              uint8_t rows[8])
{
    for(int r=0;r<8;r++){
        uint8_t v=0;
        for(int c=0;c<8;c++){
            if(b[r][c]==SHIP || b[r][c]==HIT)
                v |= (uint8_t)(1u<<c);
        }
        rows[r]=v;
    }
}

static void boardToRowsOpponent(const BATTLESHIP_STATUS_Type b[8][8],
                                uint8_t rows[8], bool blinkOn)
{
    for(int r=0;r<8;r++){
        uint8_t v=0;
        for(int c=0;c<8;c++){
            BATTLESHIP_STATUS_Type st=b[r][c];
            if(st==SHIP || st==HIT)
                v |= (uint8_t)(1u<<c);
            else if(st==MISS && blinkOn)
                v |= (uint8_t)(1u<<c);
        }
        rows[r]=v;
    }
}

static void placeShipRandom(BATTLESHIP_STATUS_Type b[8][8], int len){
    for(int tries=0; tries<100; tries++){
        bool vertical = (BS_Hal_GetRandom() & 1u) != 0u;
        int r = vertical
                ? (int)(BS_Hal_GetRandom() % (8-len+1))
                : (int)(BS_Hal_GetRandom() % 8);
        int c = vertical
                ? (int)(BS_Hal_GetRandom() % 8)
                : (int)(BS_Hal_GetRandom() % (8-len+1));
        bool free=true;
        for(int k=0;k<len;k++){
            int rr=r+(vertical?k:0);
            int cc=c+(vertical?0:k);
            if(b[rr][cc]!=WATER){free=false;break;}
        }
        if(!free) continue;
        for(int k=0;k<len;k++){
            int rr=r+(vertical?k:0);
            int cc=c+(vertical?0:k);
            b[rr][cc]=SHIP;
        }
        return;
    }
}

// ========= Render oponente (dev2) =========

static void drawOpponentFrame(bool blinkOn){
    if(!opponent_exists){
        uint8_t NP_rows[8] = {
            0b10000001, 0b11000001, 0b10100001, 0b10010001,
            0b10001001, 0b00011110, 0b00010001, 0b00011110
        };
        MAX_DrawRows(BS_DEV_OPPONENT, NP_rows);
        return;
    }
    uint8_t rows[8];
    boardToRowsOpponent(opponent_board, rows, blinkOn);
    MAX_DrawRows(BS_DEV_OPPONENT, rows);
}

static void genOpponentBoard(void){
    boardClear(opponent_board);
    placeShipRandom(opponent_board, 2);
    placeShipRandom(opponent_board, 4);
    placeShipRandom(opponent_board, 6);
    opponent_exists = true;
    drawOpponentFrame(true);
}

// ========= Render jugador (dev1) y pivote (dev0) =========

static void renderPlayerDev1(void){
    boardToRowsPlayer(player_board, player_rows);
    MAX_DrawRows(BS_DEV_PLAYER, player_rows);
}

// redibuja dev0: base = tablero jugador, + preview de barco si corresponde
static void renderDev0_withPreview(bool drawPreview){
    for(int r=0;r<8;r++)
        piv_rows[r]=player_rows[r];

    if(drawPreview && !all_ships_placed){
        uint8_t len = SHIP_LIST[ship_index];
        for(uint8_t k=0;k<len;k++){
            int rr=prow+(cur_ori==ORI_V?(int)k:0);
            int cc=pcol+(cur_ori==ORI_H?(int)k:0);
            if(rr>=0 && rr<8 && cc>=0 && cc<8)
                piv_rows[rr] |= (uint8_t)(1u<<cc);
        }
    }
    MAX_DrawRows(BS_DEV_PIVOT, piv_rows);
}

// ========= Movimiento / validación =========

static bool canPlaceSegment(int r,int c,ORI_t ori,uint8_t len,
                            BATTLESHIP_STATUS_Type board[8][8],
                            bool checkOverlap)
{
    if(ori==ORI_H){
        if(c<0 || (c+(int)len-1)>7 || r<0 || r>7) return false;
    }else{
        if(r<0 || (r+(int)len-1)>7 || c<0 || c>7) return false;
    }
    if(!checkOverlap) return true;
    for(uint8_t k=0;k<len;k++){
        int rr=r+(ori==ORI_V?(int)k:0);
        int cc=c+(ori==ORI_H?(int)k:0);
        if(board[rr][cc]!=WATER) return false;
    }
    return true;
}

static bool moveLeft (int *r,int *c, ORI_t ori,uint8_t len,
                      BATTLESHIP_STATUS_Type b[8][8], bool chk)
{
    int nr=*r, nc=*c-1;
    if(canPlaceSegment(nr,nc,ori,len,b,chk)){*r=nr;*c=nc;return true;}
    return false;
}
static bool moveRight(int *r,int *c, ORI_t ori,uint8_t len,
                      BATTLESHIP_STATUS_Type b[8][8], bool chk)
{
    int nr=*r, nc=*c+1;
    if(canPlaceSegment(nr,nc,ori,len,b,chk)){*r=nr;*c=nc;return true;}
    return false;
}
static bool moveUp   (int *r,int *c, ORI_t ori,uint8_t len,
                      BATTLESHIP_STATUS_Type b[8][8], bool chk)
{
    int nr=*r+1, nc=*c;
    if(canPlaceSegment(nr,nc,ori,len,b,chk)){*r=nr;*c=nc;return true;}
    return false;
}
static bool moveDown (int *r,int *c, ORI_t ori,uint8_t len,
                      BATTLESHIP_STATUS_Type b[8][8], bool chk)
{
    int nr=*r-1, nc=*c;
    if(canPlaceSegment(nr,nc,ori,len,b,chk)){*r=nr;*c=nc;return true;}
    return false;
}

// ========= Colocación y blink jugador =========

static void commitShip(uint8_t len){
    for(uint8_t k=0;k<len;k++){
        int rr=prow+(cur_ori==ORI_V?(int)k:0);
        int cc=pcol+(cur_ori==ORI_H?(int)k:0);
        player_board[rr][cc]=SHIP;
    }
    renderPlayerDev1();
    renderDev0_withPreview(false);
}

// ========= Blink de error (no bloqueante) =========

static void startErrorBlink(uint32_t nowMs){
    errBlink_active   = true;
    errBlink_togglesRemaining = ERR_BLINK_TOGGLES;
    errBlink_last     = nowMs;
    errBlink_showBase = false; // primero muestro preview
}

// En BS_AnimationsUpdate se maneja este blink

// ========= Disparos y victoria =========

static SHOT_RESULT_t applyShotAtBoard(BATTLESHIP_STATUS_Type b[8][8],
                                      int r,int c)
{
    BATTLESHIP_STATUS_Type st=b[r][c];
    if(st==SHIP){ b[r][c]=HIT;  return SHOT_HIT_RES; }
    if(st==WATER){b[r][c]=MISS; return SHOT_MISS_RES;}
    return SHOT_REPEAT;
}

static bool boardAllShipsDestroyed(const BATTLESHIP_STATUS_Type b[8][8]){
    for(int r=0;r<8;r++)
        for(int c=0;c<8;c++)
            if(b[r][c]==SHIP) return false;
    return true;
}

// ========= Contador (dev3) =========

static void drawDigit(uint8_t dev,uint8_t num){
    if(num>9) return;
    for(uint8_t r=0;r<8;r++){
        MAX_SetRow(dev, 7-r, digits[num][r]);
    }
}

// ========= API pública =========

void BS_GameInit(void){
    MAX_InitAll();

    boardClear(player_board);
    boardClear(opponent_board);
    genOpponentBoard();

    prow=3; pcol=0;
    cur_ori=ORI_H;
    ship_index=0;
    all_ships_placed=false;
    mode=MODE_PLACE;

    blink_enabled=false;
    blink_state=false;
    last_blink=BS_Hal_GetMillis();

    errBlink_active=false;
    errBlink_togglesRemaining=0;

    oppBlinkState=true;
    oppLastBlink=BS_Hal_GetMillis();

    current_digit=9;
    drawDigit(BS_DEV_COUNTER,current_digit);

    renderPlayerDev1();
    renderDev0_withPreview(true);
}

BS_Mode BS_GetMode(void){
    return mode;
}

void BS_AnimationsUpdate(uint32_t nowMs){
    // 1) Blink tablero jugador cuando todos los barcos colocados
    if(blink_enabled && mode==MODE_PLACE){
        if(nowMs - last_blink >= BLINK_MS){
            last_blink = nowMs;
            blink_state = !blink_state;
            if(blink_state){
                MAX_Clear(BS_DEV_PLAYER);
            }else{
                MAX_DrawRows(BS_DEV_PLAYER, player_rows);
            }
        }
    }

    // 2) Blink de MISSES del oponente
    if(opponent_exists){
        if(nowMs - oppLastBlink >= OPP_BLINK_MS){
            oppLastBlink = nowMs;
            oppBlinkState = !oppBlinkState;
            drawOpponentFrame(oppBlinkState);
        }
    }

    // 3) Blink de error al intentar colocar barco inválido
    if(errBlink_active){
        if(nowMs - errBlink_last >= ERR_BLINK_MS){
            errBlink_last = nowMs;
            if(errBlink_togglesRemaining>0){
                errBlink_togglesRemaining--;
                errBlink_showBase = !errBlink_showBase;
                if(errBlink_showBase){
                    // mostrar solo tablero base en dev0
                    MAX_DrawRows(BS_DEV_PIVOT, player_rows);
                }else{
                    // mostrar preview normal otra vez
                    renderDev0_withPreview(true);
                }
            }else{
                // Fin del blink de error: dejo preview consistente
                errBlink_active=false;
                renderDev0_withPreview(true);
            }
        }
    }
}

// ====== Contador ======

void BS_CountdownSet(uint8_t startValue){
    if(startValue>9) startValue=9;
    current_digit=startValue;
    drawDigit(BS_DEV_COUNTER,current_digit);
}

bool BS_CountdownStep(void){
    if(current_digit==0){
        // ya está en 0, no baja más
        drawDigit(BS_DEV_COUNTER,current_digit);
        return true; // ya terminado
    }
    current_digit--;
    drawDigit(BS_DEV_COUNTER,current_digit);
    return (current_digit==0);
}

// ====== FASE DE COLOCACIÓN ======

bool BS_Placement_MoveCursor(BS_Dir dir){
    if(mode!=MODE_PLACE) return false;
    uint8_t len = SHIP_LIST[ship_index];
    bool moved=false;
    switch(dir){
    case BS_DIR_LEFT:
        moved = moveLeft(&prow,&pcol,cur_ori,len,player_board,false);
        break;
    case BS_DIR_RIGHT:
        moved = moveRight(&prow,&pcol,cur_ori,len,player_board,false);
        break;
    case BS_DIR_UP:
        moved = moveUp(&prow,&pcol,cur_ori,len,player_board,false);
        break;
    case BS_DIR_DOWN:
        moved = moveDown(&prow,&pcol,cur_ori,len,player_board,false);
        break;
    }
    if(moved){
        renderDev0_withPreview(true);
    }
    return moved;
}

bool BS_Placement_RotateCursor(void){
    if(mode!=MODE_PLACE || all_ships_placed) return false;
    uint8_t len = SHIP_LIST[ship_index];
    ORI_t newOri = (cur_ori==ORI_H ? ORI_V : ORI_H);
    if(canPlaceSegment(prow,pcol,newOri,len,player_board,false)){
        cur_ori = newOri;
        renderDev0_withPreview(true);
        return true;
    }
    return false;
}

BS_PlaceResult BS_Placement_TryPlaceCurrentShip(uint32_t nowMs){
    if(mode!=MODE_PLACE) return BS_PLACE_INVALID;
    if(all_ships_placed) return BS_PLACE_ALL_DONE;

    uint8_t len = SHIP_LIST[ship_index];
    if(!canPlaceSegment(prow,pcol,cur_ori,len,player_board,true)){
        // no bloqueante: arranca blink de error
        startErrorBlink(nowMs);
        return BS_PLACE_INVALID;
    }

    // colocar barco
    commitShip(len);

    if(ship_index < 2){
        ship_index++;
        renderDev0_withPreview(true);
        return BS_PLACE_OK;
    }else{
        // último barco colocado
        all_ships_placed=true;
        blink_enabled=true;
        last_blink=nowMs;
        renderDev0_withPreview(false);
        return BS_PLACE_ALL_DONE;
    }
}

// ====== Transición a disparos ======

void BS_EnterShotMode(void){
    if(mode!=MODE_PLACE) return;
    // apaga blink de jugador
    blink_enabled=false;
    MAX_DrawRows(BS_DEV_PLAYER, player_rows);

    // Cursor de 1 bit en dev0
    for(int r=0;r<8;r++) piv_rows[r]=0;
    prow=3; pcol=0;
    piv_rows[prow] |= (uint8_t)(1u<<pcol);
    MAX_DrawRows(BS_DEV_PIVOT, piv_rows);

    mode = MODE_SHOT;
}

// ====== FASE DE DISPARO ======

bool BS_Shot_MoveCursor(BS_Dir dir){
    if(mode!=MODE_SHOT) return false;
    bool moved=false;
    switch(dir){
    case BS_DIR_LEFT:
        moved = moveLeft(&prow,&pcol,ORI_H,1,player_board,false);
        break;
    case BS_DIR_RIGHT:
        moved = moveRight(&prow,&pcol,ORI_H,1,player_board,false);
        break;
    case BS_DIR_UP:
        moved = moveUp(&prow,&pcol,ORI_H,1,player_board,false);
        break;
    case BS_DIR_DOWN:
        moved = moveDown(&prow,&pcol,ORI_H,1,player_board,false);
        break;
    }
    if(moved){
        for(int r=0;r<8;r++) piv_rows[r]=0;
        piv_rows[prow] |= (uint8_t)(1u<<pcol);
        MAX_DrawRows(BS_DEV_PIVOT, piv_rows);
    }
    return moved;
}

SHOT_RESULT_t BS_Shot_FireAtCursor(void){
    if(mode!=MODE_SHOT) return SHOT_NONE;
    SHOT_RESULT_t res = applyShotAtBoard(opponent_board, prow, pcol);
    drawOpponentFrame(true);
    return res;
}

bool BS_Shot_OpponentAllDestroyed(void){
    return boardAllShipsDestroyed(opponent_board);
}
